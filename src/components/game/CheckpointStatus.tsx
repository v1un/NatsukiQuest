...
const CheckpointStatus: React.FC<CheckpointStatusProps> = ({ gameState }) => {
    ...
    const calculatePotentialLosses = (): RbDLoss[] => {
        ...
        const newQuests = gameState.activeQuests?.filter(quest =>
            !checkpoint.activeQuests?.some(checkQuest => checkQuest.id === quest.id)
        ) || [];
        if (newQuests.length > 0) {
            losses.push({
                type: 'quest',
                description: `${newQuests.length} quest${newQuests.length > 1 ? 's' : ''} started`,
                details: newQuests.map(quest => quest.title).join(', '),
                severity: 'moderate'
            });
        }

        // Check skill gains
        const newSkills = gameState.skills.filter(skill =>
            !checkpoint.skills.some(checkSkill => checkSkill.id === skill.id)
        );
        if (newSkills.length > 0) {
            losses.push({
                type: 'skill',
                description: `${newSkills.length} skill${newSkills.length > 1 ? 's' : ''} gained`,
                details: newSkills.map(skill => skill.name).join(', '),
                severity: newSkills.length > 2 ? 'major' : 'moderate'
            });
        }
        ...
        // Check knowledge gains
        const newLore = gameState.discoveredLore?.filter(loreId =>
            !checkpoint.discoveredLore?.includes(loreId)
        ) || [];
        if (newLore.length > 0) {
            losses.push({
                type: 'knowledge',
                description: `${newLore.length} lore entr${newLore.length > 1 ? 'ies' : 'y'} discovered`,
                details: `${newLore.length} piece${newLore.length > 1 ? 's' : ''} of knowledge`,
                severity: newLore.length > 3 ? 'major' : 'moderate'
            });
        }
    ...
    const checkpointAge = (gameState && gameState.checkpoint) ? 
        gameState.memory.split('\n').length - gameState.checkpoint.memory.split('\n').length : 0;
